#!/usr/bin/env python3
"""
Create video visualization using the BEST model (highest dice score)
small_unet_manual_trained.pt - Dice: 0.5070
"""
...
def create_detection_video_best():
    """Process AIH_Bubbles3.mp4 with best trained model"""

    # Paths
    base_dir = Path(__file__).parent.parent.parent
    video_path = base_dir / "videos" / "AIH_Bubbles3.mp4"
    model_path = base_dir / "data" / "cnn" / "small_unet_manual_trained.pt"  # BEST MODEL
    output_dir = base_dir / "data" / "validation_bubbles3"
    output_video = output_dir / "AIH_Bubbles3_detected_BEST.mp4"
    ...
    model.eval()
    print(f"Model loaded: BEST Dice Score = 0.5070 (50.70%)")
    ...
    print("\nProcessing frames with BEST model (Dice: 0.5070)...")
    ...
    while True:
        ...
        # Draw mask overlay (cyan for BEST model)
        mask_overlay = np.zeros_like(vis)
        mask_overlay[binary_mask > 0] = [255, 255, 0]  # Cyan
        vis = cv2.addWeighted(vis, 0.7, mask_overlay, 0.3, 0)

        # Draw bounding boxes
        for contour in valid_contours:
            x, y, w_box, h_box = cv2.boundingRect(contour)
            cv2.rectangle(vis, (x, y), (x + w_box, y + h_box), (255, 255, 0), 2)

        # Add text overlay
        text = f"Frame {frame_idx} | Bubbles: {num_bubbles} | BEST MODEL (Dice: 0.5070)"
    ...
    print(f"\n{'='*60}")
    print(f"Video processing complete (BEST MODEL)!")
    print(f"  Model: small_unet_manual_trained.pt")
    print(f"  Dice Score: 0.5070 (50.70%)")
...
if __name__ == "__main__":
    output = create_detection_video_best()
    if output:
